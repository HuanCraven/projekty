...
VERSION = "2026.08.30-03"  # při každém buildu zvyš (RRRR.MM.DD-NN)
...
<div id="navod" style="display:none">
  ...
  <dl class="nav-dl">
    ...
    <dt>Rejstřík kamenů</dt><dd>Obrácený pohled — vyjdete od stavebního kamene ScioCíle
      ...
      konkrétní kámen.</dd>
    <dt>Hlasování</dt><dd>Výběr témat, která chcete vést.
...
<div id="hlas" style="display:none">
  ...
  Garantství rozděluje co nejrovnoměrněji. Nic nikam nezapíše — je to jen podklad.</p>
...
function render(){
  ...
  if(view==="hlas"){
    if(!hlasInit){ hlasInit=true; initHlas(); } else { loadMyVotes(); }
    return;
  }
  const nq=norm(q);
...
async function toggleNavrh(){
  ...
  const pocetRez = vybrane.filter(v=>v.rezervace).length;
  if(pocetRez) html += `<br>Z dřívějška vstoupily do návrhu <b>${pocetRez}</b> rezervace — ty jsou brané jako dané a rozdělení garantů se počítá kolem nich. Rezervace se už v hlasování nedají zadávat.`;
